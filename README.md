...
```
git clone https://github.com/ekaneeraj/dao-contract
cd dao-contract
forge build
```
...
a. DeployDAO
```
forge script script/DeployDAO.s.sol --rpc-url $SEPOLIA_RPC_URL --private-key $PRIVATE_KEY --broadcast --verify --etherscan-api-key $ETHERSCAN_API_KEY
```
...
```bash
forge script script/DeployDAO.s.sol --rpc-url $SEPOLIA_RPC_URL --account defaultKey --sender <Wallet Address> --broadcast`
```
